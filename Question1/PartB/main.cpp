...
public:
    ...
    double determinant() const {
        ...
        for (int i = 0; i < n; i = i + 1) {
            ...
            for (int r = i + 1; r < n; r = r + 1) {
                if (abs(temp.mData[r][i]) > abs(temp.mData[pivot][i])) {
                    pivot = r;
                }
            }
            if (abs(temp.mData[pivot][i]) < 1e-9) {
                return 0.0;
            }
            if (pivot != i) {
    ...
    Matrix inverse() const {
        ...
        for (int i = 0; i < n; i = i + 1) {
            int pivot = i;
            for (int r = i + 1; r < n; r = r + 1) {
                if (abs(aug.mData[r][i]) > abs(aug.mData[pivot][i])) {
                    pivot = r;
                }
            }
            assert(abs(aug.mData[pivot][i]) > 1e-9);
            if (pivot != i) {
...
int main() {
    ...
    int testRows = 42;
    int features = 6;
    Matrix A_train(trainRows, features);
    Matrix b_train(trainRows, 1);
    Matrix A_test(testRows, features);
    Matrix b_test(testRows, 1);
    ...
    if (file.is_open()) {
        while (getline(file, line)) {
            ...
            getline(ss, token, ',');
            getline(ss, token, ',');
            if (currentRow <= trainRows) {
                for (int j = 1; j <= features; j = j + 1) {
                    getline(ss, token, ',');
                    A_train(currentRow, j) = stod(token);
                }
                getline(ss, token, ',');
                b_train(currentRow, 1) = stod(token);
            } else {
                int testRowIndex = currentRow - trainRows;
                for (int j = 1; j <= features; j = j + 1) {
                    getline(ss, token, ',');
                    A_test(testRowIndex, j) = stod(token);
                }
                getline(ss, token, ',');
                b_test(testRowIndex, 1) = stod(token);
            }
            currentRow = currentRow + 1;
        }
        file.close();
    } else {
        cout << "Khong the mo file machine.data" << endl;
        return 0;
    }
    cout << "Dang tinh toan tham so X..." << endl;
    Matrix X = A_train.pseudoInverse() * b_train;
    cout << "Cac tham so x1 den x6 la:" << endl;
    X.print();
    Matrix b_pred = A_test * X;
    double sumSquaredError = 0.0;
    for (int i = 1; i <= testRows; i = i + 1) {
        double error = b_pred(i, 1) - b_test(i, 1);
        sumSquaredError = sumSquaredError + (error * error);
    }
    double mse = sumSquaredError / testRows;
    double rmse = sqrt(mse);
    cout << "Do loi RMSE tren tap kiem tra (Test Set) la: " << rmse << endl;
    return 0;
}